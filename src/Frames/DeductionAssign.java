 ...
 * @author Sapu
 */
public class DeductionAssign extends javax.swing.JFrame {

    /**
     * Creates new form AllowanceAssign
     */
    public DeductionAssign() {
        initComponents();
        auto_completer();
        get_data_to_table();
        TitleBar();
        txt_AllowAssign_RankCode.setEditable(false);
        table_deductionAssign.getTableHeader().setDefaultRenderer(new Allowances.HeaderColor());
        table_deductionAssign.setFont(new Font("Times New Roman", Font.PLAIN, 14));
    }
    ...
    private void table_clear() {
        DefaultTableModel dtm = (DefaultTableModel) table_deductionAssign.getModel();
        dtm.setRowCount(0);
    }
    ...
    private void get_data_to_table() {
        try {
            ...
            if (table_deductionAssign.getRowCount() == 0) {
                
                Statement st = DbConnection.getconnection().createStatement();
                ResultSet rs = st.executeQuery("SELECT rankwise_deduction.DeductionCode, deductions_reg.name, deductions_reg.amount FROM rankwise_deduction INNER JOIN deductions_reg ON rankwise_deduction.DeductionCode=deductions_reg.code WHERE RankCode='" + txt_AllowAssign_RankCode.getText() + "'");
                while (rs.next()) {
                    
                    DefaultTableModel dtm = (DefaultTableModel) table_deductionAssign.getModel();
                    Vector v = new Vector();
                    
                    v.add(rs.getString("DeductionCode"));
    ...
    private void initComponents() {
        ...
        table_deductionAssign = new javax.swing.JTable();
        ...
        jLabel1.setText("Deduction Assign - Rank-wise");
        ...
        jLabel6.setText("Deduction Code :-");
        ...
        jLabel7.setText("Deduction  Name :-");
        ...
        table_deductionAssign.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {

            },
            new String [] {
                "Deduction Code", "Deduction Name", "Amount"
            }
        ) {
        ...
        table_deductionAssign.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                table_deductionAssignMouseClicked(evt);
            }
        });
        jScrollPane1.setViewportView(table_deductionAssign);
    ...
    private void btn_applyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_applyActionPerformed
        ...
        if (txt_allowanceCode.getText().isEmpty() || txt_allowanceAmount.getText().isEmpty()) {
            ...
            t.DeductionCode = txt_allowanceCode.getText();
            t.DeductionName = txt_allowanceName.getText();
            t.DeductionAmount = txt_allowanceAmount.getText();
        ...
        try {
            ...
            if (txt_AllowAssign_RankCode.getText().isEmpty() || txt_allowanceCode.getText().isEmpty()) {
                ...
                for (table_data td : al) {
                    ...
                    st.executeUpdate("insert into rankwise_deduction values('" + txt_AllowAssign_RankCode.getText() + "','" + td.DeductionCode + "')");
    ...
    private void btn_removeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_removeActionPerformed
        ...
        if (table_deductionAssign.getSelectedRowCount() == 0) {
            ...
            try {
                ...
                Statement st = DbConnection.getconnection().createStatement();
                st.executeUpdate("delete from rankwise_deduction where DeductionCode='" + txt_allowanceCode.getText() + "'");
    ...
    private void btn_updateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_updateActionPerformed
        try {
            ...
            Statement st = DbConnection.getconnection().createStatement();
            st.execute("delete from rankwise_deduction where RankCode='" + txt_AllowAssign_RankCode.getText() + "' && AllowanceCode='" + txt_allowanceCode.getText() + "'");
...
//        txt_AllowAssign_RankName.setText("");
         ...
         try {
                ...
                JasperDesign jd = JRXmlLoader.load("Reports\\VO_Wise_EMP_rpt.jrxml");
                String sql = "SELECT employee_reg.`EPFno` AS employee_reg_EPFno,employee_reg.`FullName` AS employee_reg_FullName,employee_reg.`NameWithInitials` AS employee_reg_NameWithInitials,employee_reg.`DefLocation` AS employee_reg_DefLocation,employee_reg.`Designation`  ,location_reg.`LocCode` AS location_reg_LocCode,location_reg.`LocName` AS location_reg_LocName,location_reg.`LocInchargeName` AS location_reg_LocInchargeName,location_reg.`LocInchargeID` AS location_reg_LocInchargeID FROM `employee_reg` employee_reg, `location_reg` location_reg WHERE employee_reg.`DefLocation` = location_reg.`LocCode` AND location_reg.`LocInchargeID`='510' ORDER BY  employee_reg.`DefLocation`";
    ...
    private void table_deductionAssignMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_table_deductionAssignMouseClicked
        try {
            
            int row = table_deductionAssign.getSelectedRow();
            String get_code = table_deductionAssign.getModel().getValueAt(row, 0).toString();
    ...
    }//GEN-LAST:event_table_deductionAssignMouseClicked
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(DeductionAssign.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new DeductionAssign().setVisible(true);
            }
        });
    }
    ...
    private javax.swing.JTable table_deductionAssign;
